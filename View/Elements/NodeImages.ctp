<div class="row-fluid padded">
    ...
    $(function(){
      ...
      $(".Image-add").click(function(){
        ...
        imagepicker.getFiles(function(files){
          console.log(files);
          for (file in files) {
            var url = "<?php echo $this->Html->url(array("plugin"=>"node_images","controller"=>"node_images","action"=>"add"));?>?url=" + 
    ...
    <tbody>
      ...
      if(isset($this->data['NodeImage'])) {
        ...
        foreach($this->data['NodeImage'] as $Image) {
          $this->NodeImages->row($Image['title'], $Image['url'], $Image['id'], $i);
          $i++;
        }
      }
      ?>
  ...
  <button class="btn Image-add" onclick="return false;">Browse</button>
</div>
